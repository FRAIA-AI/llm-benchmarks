...
async def run_benchmark():
    ...
    # 2. Load Config
    try:
        with open("/app/configs/workloads.json", "r") as f:
            full_config = json.load(f)
            config = full_config[TEST_TYPE]
    except KeyError:
        print(f"!!! Invalid TEST_TYPE: {TEST_TYPE} !!!")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            if time.time() - start_wait > 600: # 10 minute timeout for model load
